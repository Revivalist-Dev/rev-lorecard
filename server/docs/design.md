...
*   **Purpose**: Represents a single web source (e.g., a wiki category page) that can be crawled for links. A `Project` can have multiple `ProjectSource`s.
...
*   **Fields**:
    ...
    *   `max_pages_to_crawl` (Integer): The maximum number of pages to follow using the pagination selector.
...
*   **Fields**:
    ...
    *   `task_name` (Text): The name of the task to be executed (e.g., `generate_search_params`, `generate_selector`, `confirm_links`, `process_project_entries`).
...
##### **2.8. Database Schema & Relationships**
...
##### **3.2. Workflow B: Selector Generation & Crawling**

1.  **Job Invocation**: The user selects one or more `ProjectSource`s and initiates this step. This creates a `BackgroundJob` with `task_name: 'generate_selector'`.
2.  **Worker Execution**: A background worker picks up the job and iterates through the selected sources.
    *   For each `ProjectSource`, it scrapes the raw HTML from its `url`.
    *   It constructs a detailed prompt using the `templates.selector_generation` template, including the scraped HTML and the structured `search_params`.
    *   It sends the prompt to the LLM, asking for a list of CSS selectors that target article links and an optional pagination selector.
    *   It creates an `ApiRequestLog` for this call.
    *   Upon receiving a valid response, the worker updates the `ProjectSource` with the new selectors.
    *   It then immediately crawls the source (and subsequent pages if a pagination selector was found) to discover links.
    *   The job result contains all newly found URLs.
...
*   **Background Jobs**
    ...
    *   `POST /jobs/generate-selector`: Create a job to generate CSS selectors and crawl sources (Workflow B).
    ...
    *   `POST /jobs/rescan-links`: Create a job to re-crawl sources using existing selectors.
