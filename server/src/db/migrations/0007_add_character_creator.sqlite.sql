-- SQLite requires table rebuilds for adding columns with defaults/constraints.
PRAGMA foreign_keys=OFF;

-- == Part 1: Rebuild Project Table to add project_type ==
ALTER TABLE "Project" RENAME TO "Project_temp_for_type";
...
CREATE TABLE "Project" (
    ...
    "project_type" TEXT NOT NULL DEFAULT 'lorebook',
    FOREIGN KEY ("credential_id") REFERENCES "Credential"("id") ON DELETE SET NULL
);
INSERT INTO "Project" (id, name, prompt, search_params, templates, status, requests_per_minute, created_at, updated_at, credential_id, model_name, model_parameters)
SELECT id, name, prompt, search_params, templates, status, requests_per_minute, created_at, updated_at, credential_id, model_name, model_parameters FROM "Project_temp_for_type";
DROP TABLE "Project_temp_for_type";
CREATE INDEX IF NOT EXISTS "ix_project_project_type" ON "Project" ("project_type");
...
-- == Part 2: Alter ProjectSource Table ==
ALTER TABLE "ProjectSource" ADD COLUMN "raw_content" TEXT;
ALTER TABLE "ProjectSource" ADD COLUMN "content_type" TEXT;
ALTER TABLE "ProjectSource" ADD COLUMN "content_char_count" INTEGER;


-- == Part 3: Rebuild ALL dependent tables to update their Foreign Keys to the new Project table ==
...
CREATE TABLE "ProjectSource" (
    ...
    "raw_content" TEXT, "content_type" TEXT, "content_char_count" INTEGER,
    FOREIGN KEY ("project_id") REFERENCES "Project"("id") ON DELETE CASCADE,
    UNIQUE ("project_id", "url")
);
...
INSERT INTO "ProjectSource" (id, project_id, url, link_extraction_selector, link_extraction_pagination_selector, max_pages_to_crawl, last_crawled_at, created_at, updated_at, max_crawl_depth, raw_content, content_type, content_char_count)
SELECT id, project_id, url, link_extraction_selector, link_extraction_pagination_selector, max_pages_to_crawl, last_crawled_at, created_at, updated_at, max_crawl_depth, raw_content, content_type, content_char_count FROM "ProjectSource_old";
...
-- == Part 4: Create CharacterCard Table ==
...
CREATE INDEX IF NOT EXISTS "ix_charactercard_project_id" ON "CharacterCard" ("project_id");
CREATE TRIGGER IF NOT EXISTS update_charactercard_updated_at AFTER UPDATE ON "CharacterCard"
BEGIN
    UPDATE "CharacterCard" SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = OLD.id;
END;
